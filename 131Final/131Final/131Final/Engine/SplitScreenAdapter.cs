    ...
    static class SplitScreenAdapter
    {
        ...
        static public Vector2 splitConvert(int Screen, Vector2 myPoint, SpriteBatch gRep)
        {
            ...
            if(Screen == 2 || Screen == 4)
                point.X += gRep.GraphicsDevice.Viewport.Width / 2;
            if (Screen == 3 || Screen == 4)
                point.Y += gRep.GraphicsDevice.Viewport.Height / 2;
            return point;
        }
        static public void drawLines(SpriteBatch batch)
        {
            int myH = batch.GraphicsDevice.Viewport.Height / (15 + 1) / 2;
            GridManager.DrawLine(batch, myH/5, Color.Black,
                new Vector2(0, batch.GraphicsDevice.Viewport.Height / 2 - myH/3),
                new Vector2(batch.GraphicsDevice.Viewport.Width,batch.GraphicsDevice.Viewport.Height / 2 - myH/3));
            GridManager.DrawLine(batch, myH/5, Color.Black,
                new Vector2(batch.GraphicsDevice.Viewport.Width / 2,0),
                new Vector2(batch.GraphicsDevice.Viewport.Width / 2, batch.GraphicsDevice.Viewport.Height));
        }
